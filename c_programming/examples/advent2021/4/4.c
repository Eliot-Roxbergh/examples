...
static bool number_is_drawn(int number, const int *draws,
...
static int get_score(board *board, int *draws, unsigned int draw_count)
{
    ...
    // add number to score only if it's not drawn (i.e. not in excluded numbers
    // list)
...
int main()
{
    ...
    int *drawn_numbers =
        input_ints[0].elems;  // first line must be drawn numbers
    ...
    unsigned int nr_of_boards =
        lines_in_raw_input / 5;  // theoretical max nr of boards
        ...
        if (input_ints[cur_line].nr_elems != 5) {
            ...
            for (unsigned int row = 0; row < 5; row++) {
                for (unsigned int col = 0; col < 5; col++) {
                    ...
                    int current_num =
                        input_ints[start_of_board + row].elems[col];
    ...
    //  if bingo is found, still check all boards, since another board could get
    ...
    for (unsigned int draws = 1; draws <= drawn_numbers_count; draws++) {
        for (unsigned int board_nr = 0; board_nr < nr_of_boards; board_nr++) {
            ...
            if (is_bingo) {
                ...
                // TODO should also check that winner (on same draw) has more
                // points than prev winner?
                //      (winner->draws_to_win == draws && board->score >
                //      winner->score)
                ...
                printf(
                    "Board %u got bingo after %u draws, with a score of %d\n",
                    board_nr + 1, board->draws_to_win, board->score);
            }
        }
        // if (winner) break; //for part 1 we can break here, and not check
        // every draw
    }
