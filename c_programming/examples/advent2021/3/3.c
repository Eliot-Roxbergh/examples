...
int main()
{
    ...
    // for each column (x from 0 to 11), if more than half chars in row (y) are
    // '1'
    ...
    //  since a value may only be one or zero, we simply bit invert gamma rate
    //  to get epsilon rate.
    // NOTE: if a column has the same number of '1' and '0', the result is
    // undefined
    // TODO: we get a few warnings "warning: use of a signed integer operand
    // with a binary bitwise operator [hicpp-signed-bitwise]" ( >> ^ << )
    ...
    epsilon_rate =
        gamma_rate ^
        (4096 - 1);  // invert the data bits (12 low bits: 4096-1 = 2^12-1)
    printf("i) %d*%d = %d\n", gamma_rate, epsilon_rate,
           gamma_rate * epsilon_rate);

    /* part 2 */
    enum status { UNSET = 0, OXYGEN, SCRUBBER, NONE };
    ...
    for (uint16_t x = 0; x < width; x++) {
        ...
        for (uint16_t y = 0; y < hits; y++) {
            ...
            // remove lines not matching previous runs result for oxygen and
            // scrubber
            if (x > 0) {
                // TODO well I guess this might fail for certain input since we
                // don't check the last iteration this way (only x-1)
                ...
                if (input_string_status[y] == UNSET) {
                    ...
                    // line is still OK
                } else if (actual_last_char == oxygen_last_char &&
                           input_string_status[y] == OXYGEN) {
                    oxygen_str = input_strs[y];
                } else if (actual_last_char == scrubber_last_char &&
                           input_string_status[y] == SCRUBBER) {
                    ...
                    if (input_string_status[y] == OXYGEN &&
                        oxygen_strs_matching > 1) {
                        oxygen_strs_matching--;
                        input_string_status[y] = NONE;
                    } else if (input_string_status[y] == SCRUBBER &&
                               scrubber_strs_matching > 1) {
                        scrubber_strs_matching--;
                        input_string_status[y] = NONE;
                    }
                    continue;
            ...
            if (input_string_status[y] == SCRUBBER ||
                input_string_status[y] == UNSET) {
                if (input_strs[y][x] == '0') {
                    zeroes_scrubber++;
                }
                if (input_strs[y][x] == '1') {
