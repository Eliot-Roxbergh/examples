 ...
 *      "An array of arrays (i.e. a two-dimensional array in C) decays into a pointer to an array, not a pointer to a pointer. Pointers to arrays can be confusing, and must be treated carefully"
 *  (also https://stackoverflow.com/questions/9446707/correct-way-of-passing-2-dimensional-array-into-a-function)
 */
...
void func(int (*apa)[3], unsigned int len)
{
    ...
    memcpy(my_apa, apa[0], 3*sizeof(*my_apa)); //copy data of first array
...
int main() {
    ...
    for (int x=0; x<5; x++) {
        papa[x] = alloca(3*sizeof(int)); //do five times: allocate memory for 3 ints and store the pointer in ptrs allocated above ("int[5]")
    }
    printf("\nPtr ptr:\n");
    for (unsigned int y=0; y<5; y++) {
        for (unsigned int x=0; x<3; x++) {
