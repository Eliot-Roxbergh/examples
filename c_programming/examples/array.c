 ...
 * applied recursively" "An array of arrays (i.e. a two-dimensional array in C)
 * decays into a pointer to an array, not a pointer to a pointer. Pointers to
 * arrays can be confusing, and must be treated carefully" (also
...
int main()
{
    ...
    int **papa =
        alloca(5 * sizeof(void *));  // allocate memory for five ptrs, which
                                     // each points to int* ("int[3]")
    for (int x = 0; x < 5; x++) {
        papa[x] = alloca(
            3 *
            sizeof(
                int));  // do five times: allocate memory for 3 ints and store
                        // the pointer in ptrs allocated above ("int[5]")
    }
    printf("\nPtr ptr:\n");
    for (unsigned int y = 0; y < 5; y++) {
        for (unsigned int x = 0; x < 3; x++) {
