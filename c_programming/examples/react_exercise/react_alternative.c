...
// invoke all callbacks on a single cell (this should be called once whenever
...
static void iterate_over_all_children(cell *c, enum iterate_action action)
{
    ...
    switch (action) {
        ...
        case COMPUTE_VALUE: {
            if (c->compute1) {
                ...
                c->new_value = c->compute2(c->parents[0]->new_value,
                                           c->parents[1]->new_value);
            } else {
        ...
        case DELETE_ALL:
            break;  // we will delete later, after recursion call, due to
                    // reverse order: from last to first child
        default:
            return;
            break;
    }
    ...
    if (action != DELETE_ALL) {
        ...
        // so basically, we iterate all the way down and then free each cell
        // upwards
        ...
        // delete my parent(s) references to me, then if they have no children
        // left free that memory (cell **children)
