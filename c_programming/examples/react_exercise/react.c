...
static void iterate_over_all_children(cell *, enum iterate_order,
                                      bool (*func)(cell *));
...
{
    // I assume zero initialization of memory should result in integer=0 and
    // points=NULL, always? ...
...
cell *create_compute2_cell(reactor *r, cell *c1, cell *c2, compute2 compute2)
{
    ...
    // add the same child pointer to its other parent (which points to same
    // child in memory)
...
void remove_callback(cell *c, callback_id id)
{
    ...
    if (c->cb_st->id == id) {
        matching_cb = c->cb_st;
        // put second element in list first (element is be NULL if matching_cb
        // was only element in list)
    ...
    while (cb_st) {
        if (cb_st->id == id) {
            matching_cb = cb_st;
            // point previous cb_st to the one after matching entry (points to
            // NULL if last)
...
/*
 * Add a child to compute cell c (in c->children), if child is null: allocate
 * and zero initialize that child. Returns child (same as the given argument
 * 'child' if it wasn't NULL)
 *
 * Each parent have a cell** children, which contains a list to its direct
 * children, this function (re-)allocates the children memory and points this
 * new area to a new child. Both children and new child
 * (children[nr_of_children-1]) need to be freed.
 */
static cell *compute_cell_add_child(cell *c, cell *child)
{
    cell **children;
    ...
    assert(c);
    assert(!(c->children &&
             c->nr_of_children == 0));  // no children but ptr not NULL
    ...
    // reallocate children
    //  c->children points to a memory area, increase this to hold one more
    //  pointer or if c->children=NULL this will initialize the memory (to hold
    //  one child pointer)
 ...
 * all_delete  - completely delete cell and all its children, also delete any
 * reference to cell from its parents all_compute - recalculate compute cells
 * all_invoke  - invoke callbacks on cells which have received a new value since
 * last callback invokation
 * */
...
/*
 * Perform supplied action on a cell, then go deeper (first a parent, then all
 * its children, and then on the children's children, etc.) Both input cell and
 * compute cell can be given as input (cell* c)
 *
 * Using depth-first search,
 *  we may either perform action then go deeper (FROM_FIRST_TO_LAST_CELL),
 *  or go deeper then perform action which results in a kind of reverse order
 * (FROM_LAST_TO_FIRST_CELL)
 *
 * (Comment: The "reverse order" is not globally reversed, but each subtree is
 * taken in reversed order, e.g. the parent 0 with two children trees 123 45
 *                   would be in order 0 123 45 but reversed 321 54 0 )
 *
 * Comment: recursion in C might blow the stack if we go too deep?
 */
static void iterate_over_all_children(cell *c, enum iterate_order order,
                                      bool (*func)(cell *))
{
    ...
    if (order == FROM_FIRST_TO_LAST_CELL) {
        is_finished = func(c);
        if (is_finished) {
            // stop iterating if we are finised. (this is just an optimisation
            // and not necessary, since the function should not do anything if
            // called when it has nothing to do)
            return;
        }
    }

    // go deeper
    for (unsigned int i = 0; c->children != NULL && i < c->nr_of_children;
         i++) {
        iterate_over_all_children(c->children[i], order, func);
    }

    if (order == FROM_LAST_TO_FIRST_CELL) {
        func(c);  // return value is unnecessary (we can't and don't want to
                  // stop early)
    }
}

/* functions used in iterate_over_all_children, returns true when iteration
 * should end */
...
static bool compute_value(cell *c)
{
    if (c->compute1) {
        ...
        c->new_value =
            c->compute2(c->parents[0]->new_value, c->parents[1]->new_value);
    } else {
...
// returns true if value hasn't changed since last time (and therefore callbacks
// are not invoked)
...
// delete everything on a single cell, and update its parent accordingly (modify
// cell->parent->children) returns true if the cell has no parents, indicating
// that the whole tree should now be empty
static bool delete_cell(cell *c)
{
    ...
    // delete my parent(s) references to me, then if they have no children left
    // free that memory (cell **children)
