 ...
 *                                           (e.g. on the stack by simple
 * declaration: struct test obj; struct test *obj_ptr = &obj;)
 *
 *    - Or we don't need to do anything and the function can store it on the
 * heap with malloc and return the pointer. By doing so it's not released when
 * that function returns, but (we) caller needs to free the memory manually.
 */
 ...
 * We only change the value locally, the parent has no way of knowing local
 * changes.
 */
void init_test(struct test *obj_ptr)
{
    struct test new_obj =
        *obj_ptr;  // only copies value, useless if we don't return it somehow
    new_obj.a = 1;

    (void)new_obj;  // compiler know the problem... above lines will otherwise
                    // result in warning (-Werror=unused-but-set-variable)
}

/* Just sets a field in the given memory address.
 * Remember this memory needs to be allocated by the caller.
 * So this function can update its values but the structure needs to already be
 * allocated elsewhere.
 */
...
/* Malloc: create presistent struct with malloc, set given pointer to this new
 * object. This is all good, just remember to free it later! Pointer could
 * either be returned via argument (set the new address in the ptr ptr given by
 * caller), or as a return value (return new_obj;)
 */
...
int main()
{
    ...
    init_test(obj_ptr);
    printf("Got %d \n", obj_ptr->a);  // not initialized or set, random value
                                      // from memory (or segfault)

    /* Uninitialized ptr is not meant to be deferenced */
    printf("This might segfault, ignoring...\n");
    /*
    struct test *obj_ptr_2; //This pointer is not initialized, we don't know if
    its value (address) is valid! init_test_2(obj_ptr_2); //possible segfault,
    dereferencing and writing to unknown memory printf("Got %d \n",
    obj_ptr_2->a);
    */
    ...
    // we need to send a ptr to that address (ptr to struct test*) so the
    // function can return it for us to read
